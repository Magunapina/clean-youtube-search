...
<main
  bind:this={mainRef}
  class="flex min-w-0 flex-1 flex-col overflow-y-auto bg-neutral-900 lg:mx-auto lg:max-w-7xl"
>
  {#if errorMessage}
    <!-- Error display -->
    <div
  ...
  {:else}
    ...
    <div class="hidden flex-1 flex-col items-center justify-center p-6 lg:flex">
      <span class="mb-3 block h-12 w-12 [&>svg]:h-full [&>svg]:w-full"
        >{@html searchIcon}</span
      >
