<script lang="ts">
  ...
  interface Props {
    ...
    required?: boolean;
    historyKey?: string;
    // Mask like type="password" without triggering password managers
    // (Firefox pairs the preceding text input as a "username" field even
    // with autocomplete="off"/"new-password")
    masked?: boolean;
  }
  ...
  // Keep the input focused when clicking dropdown buttons
  function preventBlur(e: MouseEvent) {
    e.preventDefault();
  }

  const createDropdownClick = (item: string) => () => {
    // Sync the DOM value before blur: blur fires a change event with the
    // current DOM value, which would otherwise overwrite the selection
    // with the half-typed prefix
    if (inputRef) inputRef.value = item;
    oninput(item);
    isFocused = false;
    inputRef?.blur();
  };
...
<div class="grid">
  <label for={id} class="mb-1 block">
    {label}{#if required}
      <span class="ml-2 text-red-500">*Required</span>{/if}
  </label>
  <div class="relative">
    <input
      bind:this={inputRef}
      ...
      class="w-full rounded-full bg-neutral-700 px-4 py-2 pr-11 transition-colors focus:ring-2 focus:ring-red-500 focus:outline-none {masked
